...
class StationController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if(property_exists($payload->station, 'type')) $station->type = $payload->station->type;
        if(property_exists($payload->station, 'state')) $station->state = ED::$stationStates[$payload->station->state];

        if(property_exists($payload->station, 'faction')) {
            $factionId = $system->factions->where('name', $payload->station->faction->name)->first()->id ?? 0;
